...
public class Player implements slather.sim.Player {
    ...
    public Move play(Cell player_cell, byte memory, Set<Cell> nearby_cells, Set<Pherome> nearby_pheromes) {
        ...
        Point myCells = new Point(0, 0);
		Point enemyCells = new Point(0, 0);
		Point enemyPheromes = new Point(0, 0);
		Point myPheromes = new Point(0, 0);
        for (Cell cell : nearby_cells) {
            ...
            if (cell.player == player_cell.player) myCells = add(myCells, multiply(dir, cell.getDiameter()));
			else enemyCells = add(enemyCells, multiply(dir, cell.getDiameter()));
        }
        for (Pherome pherome : nearby_pheromes) {
            ...
            if (pherome.player == player_cell.player) myPheromes = add(myPheromes, dir);
			else enemyPheromes = add(enemyPheromes, dir);
        }
		Point direction = new Point(0, 0);
		direction = add(direction, multiply(myCells, 1.0));
		direction = add(direction, multiply(enemyCells, 0.0));
		direction = add(direction, multiply(enemyPheromes, 1.0));
		if (direction.norm() < 1e-8) direction = myPheromes;
        if (direction.norm() < 1e-8) {
            double angle = gen.nextDouble() * 2 * Math.PI - Math.PI;
            //double angle = 0;
            direction = new Point(Math.cos(angle), Math.sin(angle));
        }
        direction = multiply(normalize(direction), -1);
   ...
   private static double gamma(double x) { return Math.exp(logGamma(x)); }
    ...
    private double weight(double dist, double r) {
        ...
        double poisson = Math.pow(lambda, dist) * Math.exp(-lambda) / gamma(dist);
		double exp = lambda * Math.exp( - lambda * dist);
		return exp + poisson;
    }
